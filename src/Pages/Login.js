...
//     setLoading(true);
//     try {
//       const response = await axios.post('http://intranet.higherindia.net:3006/mail-verify', { email });
...
//     }
//     try {
//       const response = await axios.post('http://intranet.higherindia.net:3006/login', { email, password, recaptchaToken}); 
...
//     // }
//     try {
//       const response = await axios.post('http://intranet.higherindia.net:3006/login', { email, password, }); // Include token in request
//       const { token, userId } = response.data;
//       if (token && userId) {
//         localStorage.setItem('token', token);
//         localStorage.setItem('userId', userId);
...
const Login = () => {
  ...
  const handleLogin = async (e) => {
    ...
    setLoading(true);
    try {
      const response = await axios.post('http://intranet.higherindia.net:3006/login', { email, password, });
      const { token, userId } = response.data;
      if (token && userId) {
        localStorage.setItem('token', token);
        localStorage.setItem('userId', userId);
