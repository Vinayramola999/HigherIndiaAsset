...
// //       try {
// //         const response = await axios.get("https://intranet.higherindia.net:8443/workflow/get");
...
// //     try {
// //       const response = await axios.delete(`https://intranet.higherindia.net:8443/workflow/delete/${id}`);
...
// //     try {
// //       const response = await axios.post("https://intranet.higherindia.net:8443/workflow/save", {
...
const [workflowUsers, setWorkflowUsers] = useState(null)
    ...
    const fetchWorkflows = async () => {
      try {
        const response = await axios.get(
          "https://intranet.higherindia.net:8443/workflow/get"
        );
        setAvailableWorkflows(response.data);
        setWorkflows(response.data); // Ensure ID is included
      } catch (error) {
  ...
  const handleDeleteWorkflow = async (id) => {
    ...
    try {
      const response = await axios.delete(
        `https://intranet.higherindia.net:8443/workflow/delete/${id}`
      );
      setWorkflows(workflows.filter((workflow) => workflow.id !== id)); // Update the workflows state
      setUpdatedOn((prev) => prev + 1); // Trigger a re-render if needed
      // alert("Workflow deleted successfully!");
    } catch (error) {
      console.error("Error deleting workflow:", error);
      alert("Error deleting workflow. Please try again.");
  ...
  const handleAddWorkflow = async () => {
    ...
    try {
      const response = await axios.post(
        "https://intranet.higherindia.net:8443/workflow/save",
        {
          workflowname: formData.workflowname,
          description: formData.description,
          createdby: formData.createdby,
  ...
  const [availableUsersForDropdown, setAvailableUsersForDropdown] = useState(null)
  ...
  const fetchWorkflowDetails = async (workflowid) => {
    ...
    try {
      ...
      const userIds = response.data?.map(data => data.userid)
      const workflowUsersData = availableUsers?.filter(u => userIds.includes(u.user_id))
      setWorkflowUsers(workflowUsersData?.map(wu => {const workflowUser = response.data?.filter(d => d.userid === wu.user_id)?.[0]; return { ...wu, workflowid: workflowUser.workflowid, workflowUserid: workflowUser.id }} ))
      setAvailableUsersForDropdown(
        availableUsers?.filter(u => !userIds.includes(u.user_id))
      )
    } catch (error) {
  ...
  const handleDeleteWorkflowUsers = async (workflowUserId) => {
    ...
    try {
      const response = await axios.delete(
        `http://higherindia.net:3006/workflow/${workflowUserId}`
      );
      setUpdatedOn(new Date())
    } catch (error) {
      console.error("Error fetching workflow details:", error);
                ...
                {filteredWorkflows.map((workflow, index) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      {/* <td className="py-3 border-b text-center">{workflow.createdby}</td> */}
                      <td className="py-3 border-b flex justify-center items-center gap-x-2">
                        <button onClick={() => handleEditWorkflow(workflow)}>
                          <FaEdit className="text-blue-500 hover:underline" />
                        </button>
                        <button
              ...
              <div className="bg-white rounded-lg p-6 shadow-lg w-11/12 md:w-3/4 lg:w-1/2">
                ...
                >
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                    ...
                    <div className="">
                    <label className="block text-gray-700 mb-1 opacity-0">SR ONLY</label>
                      <button
                        type="submit"
                        className="rounded bg-blue-500 text-white py-3 text-sm hover:bg-green-700 transition duration-300 w-full"
                      >
                        Submit
                      </button>
                    </div>
                  ...
                  <div className="flex justify-between mt-6">
                    ...
                    <button
                      type="button"
                      onClick={() => {setIsAddUserModalOpen(true); fetchWorkflowDetails(formData.workflowid)}} // Set the Add User modal to open
                      className="rounded bg-blue-600 text-white py-2 px-3 hover:bg-green-700 transition duration-300"
                    >
                      + Add User
                    </button>
                          ...
                          {workflowUsers?.map(user => {
                           return <tr key={user.email}>
                            ...
                            <button
                              ...
                              onClick={async () =>
                                {
                                  await handleDeleteWorkflowUsers(user.workflowUserid);
                                  fetchWorkflowDetails(user.workflowid)
                                }
                              }
                            >
                              <FaTrash className="text-red-500 hover:underline" />
