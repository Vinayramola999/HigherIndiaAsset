...
//       const response = await axios.post(`https://intranet.higherindia.net:8443/api/tables/insert/${selectedCategory}`, payloadToSend, {
...
//         try {
//           const url = `https://intranet.higherindia.net:8443/api/crud/delete/${selectedCategory}/${assetId}`;
...
const AssetManagementPage = () => {
  ...
  const fetchTableData = async (categoryName = selectedCategory) => {
    try {
      const response = await axios.get(
        `https://intranet.higherindia.net:8443/api/tables/table-data/${categoryName}`
      );
      setTableData({
        columns: response.data.columns || [],
        data: response.data.data || [],
  ...
  const filteredDynamicFields = dynamicFields.filter(
    (field) => field !== "unique_id" && field !== "created_at"
  );
              ...
              <table className="min-w-full table-auto border-collapse border border-gray-300">
                <thead>
                  <tr className="bg-gray-300 sticky top-0">
                    ...
                    {tableData.columns
                      .filter((column) => column !== "unique_id")
                      .map((column, index) => (
