...
//         try {   
//             const response = await axios.get(`http://higherindia.net:9898/categories/${categoryId}`);
...
const ResubmittedApproval = () => {
    ...
    const fetchCategoryName = async (categoryId) => {
        try {   
            const response = await axios.get(`http://higherindia.net:9898/api/categories/${categoryId}`);
            return response.data.name; // Assuming the category has a name property
        } catch (error) {
            console.error('Error fetching category details', error);
            return 'Unknown Category';
